...
%token <sValue> STRING
%token WHILE FOR IF THEN DISPLAY ASSIGN EXIT  PI READ LOG EXP SQRT FLOOR CEIL ABS SIN COS TAN 
%nonassoc IFX
%nonassoc ELSE
...
statement : ';' { $$ = opr(';', 2, NULL, NULL); }
          ...
          | DISPLAY expression ';' { $$ = opr(DISPLAY, 1, $2); }
          | DISPLAY STRING ';' { $$ = opr(DISPLAY, 1, cons($2)); }
          | READ VARIABLE ';' { $$ = opr(READ, 1, id($2, GET)); }
